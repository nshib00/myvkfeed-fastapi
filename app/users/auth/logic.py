...
def create_token(token_data: dict, token_type: str) -> tuple[str, datetime]:
...
def create_access_and_refresh_tokens(token_data: dict) -> TokenInfo:
    access_token, _ = create_token(token_data=token_data, token_type=ACCESS_TOKEN_TYPE)
    refresh_token, refresh_expire = create_token(token_data=token_data, token_type=REFRESH_TOKEN_TYPE)
    return TokenInfo(
...
async def create_and_save_tokens(response: Response, user: Users = Depends(get_active_current_user)) -> TokenInfo:
    user_token_data = {'sub': str(user.id)}
    tokens: TokenInfo = create_access_and_refresh_tokens(token_data=user_token_data)
...
async def authenticate_user(username: str | None, password: str | None) -> Users | None:
    ...
    if password is None or not HashPassword.verify_password(password, existing_user.hashed_password):
