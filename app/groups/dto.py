...
class GroupDTO:
    @classmethod
    async def group_dict_to_model(cls, group_dict: dict) -> Groups:
        group_model = Groups(
    ...
    @classmethod
    async def raw_groups_to_models_list(cls, groups: list[dict]) -> list[Groups]:
        group_models = []
        for group in groups:
            group_model = await cls.group_dict_to_model(group_dict=group)
