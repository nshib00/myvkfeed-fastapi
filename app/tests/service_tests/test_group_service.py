...
async def test_add_groups_list(test_user):
    ...
    await GroupService.add_groups_list(raw_groups, user_id=test_user.id)
...
async def test_add_groups_if_not_exist(groups, test_user):
    new_groups = [
        Groups(source_id=1001, title='Should not be added', user_id=test_user.id),
        Groups(source_id=3001, title='New group', user_id=test_user.id),
    ]
    await GroupService.add_groups_if_not_exist(new_groups)

    group = await GroupService.get_group_by_source_id(3001)
